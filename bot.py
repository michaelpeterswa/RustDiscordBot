...
async def get_battlemetrics(server):
    while True:
        ...
        if(json_data["data"]["attributes"]["status"] == "online"):
            ...
            await bot.change_presence(activity=discord.Game(name=game_name), status=discord.Status.dnd)
        else:
            debugPrint(offline_game_name, DEBUG)
            await bot.change_presence(activity=discord.Game(name=offline_game_name), status=discord.Status.dnd)
        await asyncio.sleep(60)

@bot.command(pass_context=True)
async def issue(ctx):
    channel = bot.get_channel(690423117952188447)
